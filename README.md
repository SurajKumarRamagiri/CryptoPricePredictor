...
*   **Flexible Forecasting**: Choose from multiple forecast horizons:
    *   1 Hour
    *   4 Hours
    *   24 Hours
*   **Advanced AI Models**:
    ...
    *   **Auto Mode**: Automatically selects the best-performing model for the chosen pair/horizon.
*   **Model Comparison**: Side-by-side comparison of LSTM and GRU performance with metrics like MAE (Mean Absolute Error) and prediction latency.
*   **Interactive Visualizations**: Dynamic, zoomable charts powered by Plotly showing historical data, predictions, and confidence intervals.
...
*   `src/`: Contains the core source code modules:
    *   `data.py`: Handles fetching data from external APIs (Binance).
    *   `preprocessing.py`: Features engineering and data scaling.
    *   `modeling.py`: Definitions for LSTM/GRU models and prediction logic.
    *   `visualization.py`: Functions generates Plotly charts and UI metrics.
...
3.  **Pick a Model**: precise which model architecture to use, or let "Auto" decide.
